...
void Configuration::SwendsenWang_LR_Clock_Bond()
{
    ...
    for (long i_Bond = 0; i_Bond < Nlist; i_Bond++)
    {
        ...
        while (true)
        {
            ...
            if (clock_index <= n_box)
            {
                ...
                {
                    // unite clusters
                    cluster.unite(j_Site, k_Site, shift_[0], shift_[1]);
    ...
    for (long i_Site = 0; i_Site < Vol; i_Site++)
    {
        ...
        int wrapping_type = cluster.wrp;
        
        obs.Ob[para.i_R0_new] = 0;
        obs.Ob[para.i_R1_new] = 0;
        obs.Ob[para.i_Rd_new] = 0;
        obs.Ob[para.i_R2_new] = 0;

        if(wrapping_type == 0) 
        {
            obs.Ob[para.i_R0_new] = 1;
        }
        else if(wrapping_type == 1)
        {
            obs.Ob[para.i_R1_new] = 1;
        }
        else if(wrapping_type == 2)
        {
            obs.Ob[para.i_R1_new] = 1;
        }
        else if(wrapping_type == 3)
        {
            obs.Ob[para.i_Rd_new] = 1;
        }
        else if(wrapping_type == 4)
        {
            obs.Ob[para.i_R2_new] = 1;
        }
