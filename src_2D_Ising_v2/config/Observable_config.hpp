...
void Observable::initObservable()  // add addBasicObser() & addCombiObser()
{
	...
	addBasicObser(para.i_R0          , "R0"             , "Wrapping R0"    							, 0     , 1.0      						);
	addBasicObser(para.i_R1          , "R1"             , "Wrapping R1"    							, 0     , 1.0     						);
	addBasicObser(para.i_Rd          , "Rd"             , "Wrapping Rd"    							, 0     , 1.0     						);
	addBasicObser(para.i_R2          , "R2"             , "Wrapping R2"    							, 0     , 1.0    						);
	addCombiObser(para.i_Rp        	 , "Rp"        		, "Critical polynomial"    			        , 0     , 1.0		      				);

	addBasicObser(para.i_R0_new      , "R0_new"         , "Wrapping R0_new"    						, 0     , 1.0      						);
	addBasicObser(para.i_R1_new      , "R1_new"         , "Wrapping R1_new"    						, 0     , 1.0     						);
	addBasicObser(para.i_Rd_new      , "Rd_new"         , "Wrapping Rd_new"    						, 0     , 1.0     						);
	addBasicObser(para.i_R2_new      , "R2_new"         , "Wrapping R2_new"    						, 0     , 1.0    						);
	addCombiObser(para.i_Rp_new      , "Rp_new"         , "Critical polynomial new"    			    , 0     , 1.0		      				);
	
	addBasicObser(para.i_Rx        	 , "Rx"        		, "Wrapping Rx"    			      			, 0     , 1.0		      				);
...
void Observable::calCombiObser()  // add Result[]
{
	...
	Result[para.i_Rp] = Ob[para.i_R2] - 2.0 * Ob[para.i_R0];

	Result[para.i_Rp_new] = Ob[para.i_R2_new] - 2.0 * Ob[para.i_R0_new];
	...
	Result[para.i_R_K] = Ob[para.i_R_E] - Ob[para.i_E] * Ob[para.i_Rx];
}
...
std::string Observable::printOutput(bool Flag)
{
	...
	str += l_jf("sigma = " + toStr(para.sigma), 22) + l_jf("beta = " + dou2str(para.beta, 10), 22) + l_jf("L = " + toStr(para.L), 22) + "\n";
